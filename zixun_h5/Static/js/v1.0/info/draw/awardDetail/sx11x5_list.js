...
$(document).ready(function() {
    var data = HCCP.info.award("24?issue=0&limit=20");
    ...
    down_pull();
    //
    $(".x11x5_items ul li").on('click', function(){ HCCP.FUNC.go(URL_INFO_KAIJIANG_SX11x5_DETAIL+"?"+$(this).attr("id"))});
    ...
    $('.wraper .x11x5_items').scroll( function(event){
        ...
        if(!flag){
            if ($(this).scrollTop() + $('.wraper .x11x5_items').height() + 10 >= $('.wraper .x11x5_items ul').height() && $(this).scrollTop() > 10) {
                var data = HCCP.info.award("23?issue="+issue+"&limit=20");
                if(data.data.length<20){
