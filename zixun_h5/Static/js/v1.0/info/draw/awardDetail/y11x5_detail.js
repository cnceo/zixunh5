$(document).ready(function() {
	var y11x5_issue = window.location.search;
	y11x5_issue = y11x5_issue.substr(1);
	var data = HCCP.info.awarddetails(21,y11x5_issue);
	if (data == undefined) {
		//alert("No Data");
		return;
	}
...
function init_data(data){
	...
	var y11x5date = HCCP.FUNC.from_unixtime(data.openTime,'standard'); //时间
	var tmp = data.openCode.replace("\|","<\/span><span>");
	var dy = y11x5date.substr(5,2)+"-"+y11x5date.substr(8,2);
	var tm = y11x5date.substr(11,2)+":"+y11x5date.substr(14,2);
	y11x5date = dy+" "+tm;
	$("hc[name=issue]").text(data.issue);
	$("hc[name=x11x5data]").text(y11x5date);
