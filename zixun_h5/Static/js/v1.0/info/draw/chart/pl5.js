  ...
  //绑定点击事件
	...
	function init_issue(issue) {
	    if(issue && issue.issue) {
	      ...
	      HCCP.cc.limitObj =issue.limit || {}	
	      HCCP.cc.diffServer = HCCP.cc.serverTime*1000 - new Date().getTime();
	      HCCP.cc.companyEndTime = issue.companyEndTime;
	      HCCP.cc.stop = issue.stop
	      if(HCCP.cc.stop){
	      	$(".submit").addClass('tingshou')
     		 $(".submit").text("停售")
	      }else{
	      	$(".submit").removeClass('tingshou')
     		 $(".submit").text("确定")
	      }
	...
	function changeDisplay(){
		...
		//初始下数据
		// console.log(HCCP.cc.betObj[playId])
	...
	function CreateLine() {
		...
		cav.empty();
		var type = HCCP.cc.type,
			arr = ['','#e12d3e','#f29233','#3d93ec','#bf6ccb','#8fb055','#5bae7f','#e34d9a']
	...
	function addSelectNum(){
	  ...
	  if(playId == 1){
	  	...
	  	HCCP.cc.betObj_[playId] = null
	  	HCCP.cc.betObj_[playId] = $.map(HCCP.cc.betObj[playId]['tuo'], function(n){
						 	 return n.join(" ")
							}).join(" | ")
	  }
	  //更新下已选内容
	...
	function limitNum(arr, obj){
	    ...
	    if(arr && arr.length){
	        var item,playId,selectStr="",selectAry = []; 
	        for (var i = arr.length - 1; i >= 0; i--) {
	            item = arr[i];
	            playId = item.playId;
	            selectStr = item.tuo || "";
	            selectStr = selectStr.replace(/ /g,"")
	            selectAry = selectStr.split("|")
	            var selectAry_ = $.map(selectAry,function(a){ 
	                return [a.split("")]; 

	            })
	            var playAry = limitObj[playId] || [];
	            for (var j = 0; j < playAry.length; j++) {//多个规定限号
	                var flag = 0;//判断5位数次数
	                var a = playAry[j].split("|")//用|拆开 a为数组
	                for(var k = 0;k < selectAry_.length;k++){//不出错为5
	                    if(selectAry_[k].indexOf(a[k]) != -1){
	                        flag ++
	                    }
	                }
	                if(flag == 5 ){//如果5位都有限号，push一波
	...
	function getSelectData(){
	  ...
	  var str_ =  HCCP.cc.betObj_[playId] || "";
	...
	function doConfirm() {
	  ...
	  //判断限号
	  var limitJ = limitNum(thisSelectData, HCCP.cc.limitObj);
	  if(!$.isEmptyObject(limitJ)){
	        var limitObj = HCCP.cc.limitObj ? HCCP.cc.limitObj:{};
	        for(var key in limitJ){//拆分组合限号信息
	            var playAry = limitObj[key] || [];
	            var limitNumStr = playAry.join('&nbsp;&nbsp;&nbsp;&nbsp;').replace(/\|/g,"|");

	            HCCP.modal.alert({
	                title:'官方限号',
	                msg:'以下组合玩法及投注号码暂不接受投注<br/>【'+ HCCP.getPlayName(HCCP.cc.lotyId, key) + '】&nbsp;'+ limitNumStr + '组合',
	                alertBtn:'返回修改'
	            })
	            break
	         }
