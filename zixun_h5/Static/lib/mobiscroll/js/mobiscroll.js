    ...
    function Scroller(elem, settings) {
        ...
        function generateWheelItems(i) {
            ...
            for (j in warr[i]) {
                ...
                html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + warr[i][j] + cal[i] + '</div></div>';
                l++;
            }
            html += '</div>';
            return html;
        ...
        function scrollToPos(time, index, manual, dir, orig) {
            ...
            if (event('validate', [dw, index, time]) !== false) {
                ...
                var stDate = new Date(s.st),
                    y = stDate.getFullYear(),
                    m = stDate.getMonth(),
                    d = stDate.getDate();
                var ltDate = new Date(s.st-2678400000),
                    ly = ltDate.getFullYear(),
                    lm = ltDate.getMonth(),
                    ld = ltDate.getDate();

                var stArr = new Array(y,m,d);
                var ltArr = new Array(ly,lm,ld);
                
                var ct = new Date(that.temp[0],that.temp[1],that.temp[2]).getTime();
                ...
                $('.dw-ul', dw).each(function (i) {
                    ...
                    if(ct+2678400000<s.st){
                        cell = $('.dw-li[data-val="' + ltArr[i] + '"]', t);
                        v = cells.index(cell);                   
                    }else if(ct>s.st){
                        cell = $('.dw-li[data-val="' + stArr[i] + '"]', t);
                        v = cells.index(cell);   
                    }
                    if (!(cell.hasClass('dw-sel')) || sc) {
                        ...
                        // console.log(t, i, v, sc ? time : 0.1, sc ? orig : undefined);
                        // console.log(sc)
        ...
        */
        that.show = function (prevAnim) {
            ...
            for (i = 0; i < s.wheels.length; i++) {
                html += '<div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><table '+ (s.display=="bottom" ? " style=\"width:100%;table-layout:fixed\" " : "" ) +'  cellpadding="0" cellspacing="0"><tr>';
                // Create wheels
            ...
            if (s.display != 'inline') {
                ...
                that.tap($('.dwb-s span', dw), function () {
                    if (that.hide(false, 'set') !== false) {
                        ...
                        val = that.val.replace(/-/g,"");
                        elm.data("date",val);

                        if(s.yesCallBack){s.yesCallBack();}
