...
$(document).ready(function() {
    var data = HCCP.info.award("13?issue=0&limit=20");
    if (data == undefined) {
        //alert("No Data");
        return;
    }
    ...
    $("body").on('click',".fc3D_items ul li", function(){ 
        if(jumpType == "zixun"){
            HCCP.FUNC.go(URL_INFO_KAIJIANG_PL3_DETAIL+"?"+$(this).attr("id")+"&jumpType=zixun");
        }else{
            HCCP.FUNC.go(URL_INFO_KAIJIANG_PL3_DETAIL+"?"+$(this).attr("id"));
        }
    });
    $("footer button").on('click', function(){ HCCP.FUNC.go(URL_TRADE_PL3); });
    var maxVal;
        ...
        $('.wraper .fc3D_items').scroll( function(event){
            ...
            if(!flag){
                if ($(this).scrollTop() + $('.wraper .fc3D_items').height() + 10 >= $('.wraper .fc3D_items ul').height() && $(this).scrollTop() > 10) {
                    var data = HCCP.info.award("13?issue="+issue+"&limit=20");
    ...
    function Gogetissue(){
        if($(".search-issue").text().length == 7){
            ...
            var data = HCCP.info.award("13?issue="+(+checkIssue+1)+"&limit=1&checkIssue=1");
            if(data.code == 200){
                $(".fc3D_items ul").html('');
                init_data2(data.data);
                $('.issue-search-close').addClass('haschecked');
                $(".fc3D_items ul li").on('click', function(){ HCCP.FUNC.go(URL_INFO_KAIJIANG_PL3_DETAIL+"?"+$(this).attr("id"));})
            }else{
                if(data.message){
