...
$(document).ready(function() {
	var data = HCCP.info.award("16?issue=0&limit=20");
	if (data == undefined) {
		//alert("No Data");
		return;
	}
	...
	//
	$("body").on('click',".qlc_items ul li", function(){ 
		if(jumpType == "zixun"){
            HCCP.FUNC.go(URL_INFO_KAIJIANG_QLC_DETAIL+"?"+$(this).attr("id")+"&jumpType=zixun");
        }else{
            HCCP.FUNC.go(URL_INFO_KAIJIANG_QLC_DETAIL+"?"+$(this).attr("id"));
        }
	})
	$("footer button").on('click', function(){ HCCP.FUNC.go(URL_TRADE_QLC); });
	//
	var issue,maxVal;
	function init_data(data){
		...
		$.each(data, function(x,item) {
			...
			issue=item.issue;
			qlcdata = item.date;
			qlcdata = qlcdata.substr(4,2)+"-"+qlcdata.substr(6,2)+"（"+getWeekByDate(qlcdata)+"）";
			tHtml = "";
			tHtml += "<li";
			if(x==0){
				tHtml += ' class="latest"'		
			...
			tHtml += "<p><span class='issue'>第"+item.issue+"期</span><span class='date'>"+qlcdata+"</span>";
			if(x==0){
			...
			tHtml += "<div class='qlc_item'><span>"+tmp.replace(reg,"</span><span>")+"</span></div>";
			$(".qlc_items ul").append(tHtml);
		});
	}
	...
	function down_pull(){
		...
		var flag=false;
		$('.wraper .qlc_items').scroll( function(event){
			//当内容滚动到底部时加载新的内容 100当距离最底部100个像素时开始加载.
			if(!flag){
				if ($(this).scrollTop() + $('.wraper .qlc_items').height() + 10 >= $('.wraper .qlc_items ul').height() && $(this).scrollTop() > 10) {
					var data = HCCP.info.award("16?issue="+issue+"&limit=20");
	...
	function Gogetissue(){
	    if($(".search-issue").text().length == 7){
	        ...
	        var data = HCCP.info.award("16?issue="+(+checkIssue+1)+"&limit=1&checkIssue=1");
	        if(data.code == 200){
                $(".qlc_items ul").html('');
                init_data2(data.data);
                $('.issue-search-close').addClass('haschecked');
                $(".qlc_items ul li").on('click', function(){ HCCP.FUNC.go(URL_INFO_KAIJIANG_QLC_DETAIL+"?"+$(this).attr("id"));})
            }else{
                if(data.message){
