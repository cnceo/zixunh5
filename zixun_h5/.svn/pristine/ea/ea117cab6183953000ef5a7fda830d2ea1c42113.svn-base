$(document).ready(function() {
	...
	var data = HCCP.info.awarddetails(12,fc3d_issue);
	if (data == undefined) {
		//alert("No Data");
		return;
	}
...
function init_data(data){
	...
	var tce=data.tryCode.replace("\|","<\/span><span>");
	...
	$(".fc3D_item").html("<span>"+tmp.replace(reg,"</span><span>")+"</span>"+"<span class='fc3D_hao'>试机号 :<span>"+tce.replace(reg,"</span><span>")+"</span>"+"</span>");
