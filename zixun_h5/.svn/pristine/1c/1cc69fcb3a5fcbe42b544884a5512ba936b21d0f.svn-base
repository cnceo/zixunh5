$(document).ready(function() {
	...
	opt.default = {
		...
		yesCallBack: function(){
			var award = HCCP.info.award(3,$(".calen").data("date"));
	...
	dataLoad(data.data);
	//
	$('.wraper .zqdc_details').scroll(function(){
		var i = 1;
	...
	$(document).on('click',".jczq_date span",shDay);
	//点击进入下一页
	$("footer button").on('click', function(){ HCCP.FUNC.go(URL_TRADE_ZQDC)});
});
...
function dataLoad(data,cBool){
	...
	//大循环
	if(data.length==0){
		$(".zqdc_details").html('<div class="no_detail"><div></div><p>暂无数据显示</p></div>')
	};
	$.each(data, function(x,item) {
		...
		if(gamedateArr[gamedate]==undefined){
			//
			if(cBool){
				$(".zqdc_details").html('<div class="jczq_date_items" id="'+gamedate+'"></div>');
			}else{
				$(".zqdc_details").append('<div class="jczq_date_items" id="'+gamedate+'"></div>');
			}
		...
		tHtml += '<div class="jczq_item"><div class="jczq_item_title">';
		...
		if(item.rq==0){
			tHtml += '<td class="title">胜平负<span class="spf_g';
			tHtml += '">'+'</span></td>';
		}else if(item.rq<0){
			tHtml += '<td class="title">胜平负<span class="spf_g spf_l';
			tHtml += '">('+item.rq+')</span></td>';
		}else if(item.rq>0){
			tHtml += '<td class="title">胜平负<span class="spf_g spf_b';
            tHtml += '">('+"+"+item.rq+')</span></td>';
		}
		...
		tHtml += '<td class="title">上下单双</td>';
		tHtml += '</tr><tr>';
		var ifNull=isEmptyObject(item.result);
		var resultkey=['1','2','3','4','5'];
