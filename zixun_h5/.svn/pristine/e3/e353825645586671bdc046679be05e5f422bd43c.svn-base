...
function getKJ(data){
	...
	if(data && data.history && data.history.length){
		...
		table += '<colgroup><col width="33.333%"/><col width="33.333%"/><col width="33.333%"/></colgroup>';
		table += '<tbody><tr><td>期号</td><td>开奖号码</td><td>形态</td></tr></tbody></table>';

		table += '<div class="wrap-common"><table class="common row1_c"><colgroup><col width="33.333%"/><col width="33.333%"/><col width="33.333%"/></colgroup>';
...
function init_issue(issue) {
    ...
    if(issue && issue.issue) {
        ...
        HCCP.k3.limit = issue.limit
        HCCP.k3.companyEndTime = issue.companyEndTime;
        HCCP.k3.stop = issue.stop
        if(HCCP.k3.stop){
...
function doConfirm() {
	...
	//限号
	var limitJ = limitNum(thisSelectData, HCCP.k3.limit);
	if(!$.isEmptyObject(limitJ)){
	    var limitObj = HCCP.k3.limit ? HCCP.k3.limit:{};
	   	var allStr = ""
		for(var key in limitJ){//拆分组合限号信息
	    	...
	    	allStr += '<br/>【'+ HCCP.getPlayName(HCCP.k3.lotyId, key) + '】&nbsp;'+ limitNumStr + '组合'
		 }
		 HCCP.modal.alert({
	   	     title:'官方限号',
	     	 msg:'以下组合玩法及投注号码暂不接受投注'+allStr,
	       	 alertBtn:'返回修改'
	   	 })
		return false;
	}
